...
export default class Keyboard extends Component {
    ...
    playKey = ({note, octave, freq}) =>{
        const osc = this.audioContext.createOscillator()
        if(!this.oscs[octave]){
            this.oscs[octave] = {}
        }
        if(this.oscs[octave][note]){
            this.stopKey({note, octave})
        }
        this.oscs[octave][note] = osc
    ...
    stopKey = ({note, octave}) =>{
        ...
        osc.stop()
        delete this.oscs[octave][note]
    }
    render(){
        return (
            <div style={{maxWidth:'989px', marginRight:'auto', marginLeft:'auto'}}>
                {createNoteTable()
                    .map( (itm, octave) =>{                    
                        return Object.keys(itm).map( (note, idx)=>{                                                        
                            const freq = itm[note]                            
                            return (
                            <KeyboardKey 
                                ref={ref =>{ this.keyRefs[note] = ref}}
                                play={this.playKey}
                                stop={this.stopKey}
                                key={note} 
                                note={note} 
